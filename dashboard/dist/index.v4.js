  ...
  function ExaGatewayPage() {
    ...
    async function testAll() {
      ...
      for (let i = 0; i < ids.length; i += 1) {
        try { const result = await post(`/test-key/${ids[i]}`); result.ok ? ok += 1 : failed += 1; }
        catch (_) { failed += 1; }
        setTestProgress({ done: i + 1, total: ids.length });
      }
      setMessage(`Test complete: ${ok} healthy, ${failed} failed`);
