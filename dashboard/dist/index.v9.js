...
(function () {
  ...
  function ExaGatewayPage() {
    ...
    async function pruneEvents() { const data = await post("/events/prune"); setMessage(`Pruned ${data.removed ?? 0} events`); refresh(); }
    async function clearEvents() { if (!confirm("Clear ALL event history? This cannot be undone.")) return; const data = await post("/events/clear"); setMessage(`Cleared ${data.removed ?? 0} events`); refresh(); }
    ...
    const stat = (label, value, color) => React.createElement("div", { style: { ...card, borderLeft: `4px solid ${color}` } }, React.createElement("div", { style: { color: "var(--color-muted-foreground)", fontSize: 12 } }, label), React.createElement("strong", { style: { fontSize: 24 } }, String(value ?? 0)));
    const fmtTime = (ts) => { if (!ts) return "-"; const d = new Date(Number(ts) * 1000); return isNaN(d.getTime()) ? "-" : d.toLocaleString(); };
    return React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 14, fontFamily: "system-ui" } },
      ...
      tab === "Overview" && React.createElement(React.Fragment, null,
        React.createElement("div", { style: { display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(150px,1fr))", gap: 10 } }, stat("Accounts", summary && summary.total_accounts, "#3b82f6"), stat("Healthy", summary && summary.healthy_accounts, "#22c55e"), stat("Disabled", summary && summary.disabled_accounts, "#6b7280"), stat("Requests", summary && summary.requests, "#8b5cf6"), stat("Errors", summary && summary.errors, "#ef4444"), stat("Success rate", `${summary && summary.success_rate || 0}%`, "#14b8a6")),
        React.createElement("div", { style: { display: "grid", gridTemplateColumns: "minmax(0,2fr) minmax(0,1fr)", gap: 10, alignItems: "start" } },
          React.createElement(ActivityChart, { data: activity }),
          React.createElement("div", { style: card }, React.createElement("h2", null, "Status"), React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 10, fontSize: 13 } },
            React.createElement("div", null, React.createElement("div", { style: { color: "var(--color-muted-foreground)", fontSize: 12 } }, "Last activity"), React.createElement("strong", null, fmtTime(summary && summary.last_activity))),
            React.createElement("div", null, React.createElement("div", { style: { color: "var(--color-muted-foreground)", fontSize: 12 } }, "Average latency"), React.createElement("strong", null, `${Math.round((summary && summary.average_latency_ms) || 0)} ms`)),
            React.createElement("div", null, React.createElement("div", { style: { color: "var(--color-muted-foreground)", fontSize: 12 } }, "Rate limits"), React.createElement("strong", null, fmt(summary && summary.rate_limits))),
            React.createElement("div", null, React.createElement("div", { style: { color: "var(--color-muted-foreground)", fontSize: 12 } }, "Health"), React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 8 } }, React.createElement("div", { style: { flex: 1, height: 8, borderRadius: 4, background: "var(--color-border)", overflow: "hidden" } }, React.createElement("div", { style: { height: "100%", width: `${summary && summary.total_accounts ? (summary.healthy_accounts / summary.total_accounts) * 100 : 0}%`, background: "#22c55e" } })), React.createElement("span", { style: { fontSize: 12 } }, `${summary && summary.healthy_accounts}/${summary && summary.total_accounts}`))))))),
