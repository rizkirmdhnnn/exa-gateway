...
(function () {
  ...
  function ExaGatewayPage() {
    ...
    const [key, setKey] = useState(""), [message, setMessage] = useState(""), [tab, setTab] = useState("Overview"), [filter, setFilter] = useState(""), [healthFilter, setHealthFilter] = useState("all"), [page, setPage] = useState(0), [loading, setLoading] = useState(false);
    ...
    function addKey() { if (!key.trim()) return; mutate("/keys", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ key: key.trim() }) }); setKey(""); }
    function exportData() { window.open(`${API}/export?days=30`, "_blank"); }
