  ...
  function ExaGatewayPage() {
    ...
    async function testAll() {
      ...
      const results = await Promise.all(ids.map(async (id) => {
        ...
        finally { done += 1; setTestProgress({ done, total: ids.length }); }
      }));
    ...
    const stat = (label, value) => React.createElement("div", { style: card }, React.createElement("div", { style: { color: "var(--color-muted-foreground)", fontSize: 12 } }, label), React.createElement("strong", { style: { fontSize: 24 } }, String(value ?? 0)));
    ...
    return React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 14, fontFamily: "system-ui" } },
      ...
      tab === "Overview" && React.createElement(React.Fragment, null,
        React.createElement("div", { style: { display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(150px,1fr))", gap: 10 } }, stat("Accounts", summary && summary.total_accounts), stat("Healthy", summary && summary.healthy_accounts), stat("Disabled", summary && summary.disabled_accounts), stat("Requests", summary && summary.requests), stat("Errors", summary && summary.errors), stat("Success rate", `${summary && summary.success_rate || 0}%`)),
